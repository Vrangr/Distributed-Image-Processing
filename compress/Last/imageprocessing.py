...
def overlay(img, img2):
    a = cv2.addWeighted(img, 0.6, img2, 0.4, 0)
    print(a.shape)
    print(img.shape)
...
def map_work_to_option( task):
    ...
    if option == 1:
        ...
        return [md, noisereduction(img)]
    elif option == 4:
        return [md, facedetect(img)]
    elif option == 5:
        img_dat = img[0]
        img_over = img[1]
        print("{} {}".format(type(img_dat),type(img_over)))
        return [md, overlay(img_dat, img_over)]

def get_folder(option):
    if option == 1:
        return 'output/edge/'
    elif option == 2:
        return 'output/median/'
    elif option == 3:
        return 'output/nonlocal/'
    elif option == 4:
        return 'output/face/'
    elif option == 5:
        return 'output/overlay/'

def main_screen(choice = 7):
    try2 = 0
    ...
    if choice == 1:
        ...
        if try2 == 1:
            print("Select the images to be overlayed")
            filename = askopenfilenames(title='Choose a file')  # show an "Open" dialog box and return the path to the selected file
        else:
            print("Select ONE overlay image")
            filename = askopenfilename(title='Choose a file')  # show an "Open" dialog box and return the path to the selected file
            try2 = 0
        return filename, choice
    elif choice == 6:
        print("Good Bye! ")
