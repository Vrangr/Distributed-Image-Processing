...
def set_client(ctx):
    ...
    client_hb = ctx.socket(zmq.ROUTER)
    client_hb.bind(addr + port_cli_hb)

    print("Polling initialization on client side")
    fe_poller = zmq.Poller()
    fe_poller.register(client, zmq.POLLIN)

    client_hb_poller = zmq.Poller()
    client_hb_poller.register(client_hb_poller, zmq.POLLIN)

    return [client, fe_poller, client_hb, client_hb_poller]
    ...
    while True:
        ...
        be_cap = len(worker_queue.queue)
            # if be_cap and len(expired_proc_task_dict) == 0:
                ...
                # print("Poller fe")
        while be_cap:
            events = dict(fe_poller.poll(100))
